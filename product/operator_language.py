...
SIMPLE_REASONS: dict[str, str] = {
    ...
    "LOW_QUALITY_SETUP": "Not a strong enough setup for this market session.",
    "NO_TRADE": "Nothing was good enough for this market session.",
    "NOT_SURFACED": "Seen by the scan, but not a final pick.",
    "STALE_RECOMMENDATION": "The saved recommendation list is too old to trade from.",
    ...
    "NO_ELIGIBLE_TRADE": "Nothing was good enough for this market session.",
    "NO_CYCLE_RECORDED": "The paper bot has not recorded a decision for the latest completed market session.",
...
def explain_opportunity(row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "cmp": row.get("cmp") if row.get("cmp") is not None else row.get("price"),
        ...
        "downside_to_stop_pct": row.get("downside_to_stop_pct"),
