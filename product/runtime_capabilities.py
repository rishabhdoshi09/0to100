...
CAPABILITIES: tuple[dict[str, Any], ...] = (
    ...
    _cap(
        "observe_only_today",
        name="Observe only today",
        plain_name="I am not participating today",
        description="Record operator intent. Paper decisions and learning continue. Live money stays locked.",
        mode=HOME_ACTION,
        control="OBSERVE_ONLY_TODAY",
        old_invocation="none — operator intent only",
        automatic_trigger="",
        canonical_owner="research.autonomy.controls",
        persisted_state="autonomy owner_state.observe_only_date",
        failure_behavior="Does not pause paper. Does not unlock live money.",
        still_requires_terminal=False,
        homepage_visible=True,
        idempotent=True,
        safe_to_retry=True,
    ...
    _cap(
        ...
        name="Simulate past decisions",
        plain_name="Test past take-vs-skip decisions",
        description="Historical decision-quality report. BACKTEST provenance. Never writes REAL_FORWARD_MARKET promotion stats.",
        mode=HOME_ACTION,
        control="SIMULATE_PAST_DECISIONS",
        old_invocation="none — new Home research action",
        automatic_trigger="",
        canonical_owner="product.decision_simulator.run_decision_simulator",
        persisted_state="logs/product/decision_simulator.json",
        failure_behavior="Missing later bars stay INCONCLUSIVE. Rejects are not P&L.",
        still_requires_terminal=False,
        read_only=True,
        homepage_visible=True,
        idempotent=True,
    ...
    _cap(
        "check_system",
        name="Check system",
        plain_name="Check system",
        description="Read-only recompute of the existing Home / startup / runtime projection. Not a second health engine.",
        mode=HOME_ACTION,
        control="CHECK_SYSTEM",
        old_invocation="GET /api/radar-home home_os.check_system",
        automatic_trigger="",
        canonical_owner="product.home_os + product.backend_control_plane",
        persisted_state="computed from existing dashboard / soak / ops / autonomy",
        failure_behavior="Missing stays missing. Live money stays locked. No new truth source.",
        still_requires_terminal=False,
        read_only=True,
        homepage_visible=True,
        idempotent=True,
