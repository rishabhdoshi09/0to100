...
def estimate_ev(signals: list[str], entry: float, stop: float,
    ...
    Returns {ev_pct, p_win, avg_win_pct, avg_loss_pct, n} or None when the
    combined evidence is thinner than min_n (no claim). Blend is
    sample-weighted so a 400-trade signal outweighs a 40-trade one."""
    if stats is None:
        stats = signal_stats()
...
def ev_rank_key(r: dict) -> tuple:
    ...
    EV claim rank above no-claim ones; within each group, EV then conviction
    decides. Use with sort(reverse=True)."""
    has_ev = r.get("ev_pct") is not None
    return (1 if has_ev else 0,
            float(r.get("ev_pct") or 0),
