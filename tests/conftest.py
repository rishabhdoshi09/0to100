...
def isolate_mutable_runtime_state(tmp_path_factory, monkeypatch, request):
    ...
    ops_snap = tmp_path_factory.mktemp("ops_status") / "operations_status.json"
    monkeypatch.setenv("QT_OPERATIONS_SNAPSHOT", str(ops_snap))
    ops_db = tmp_path_factory.mktemp("ops_db") / "jobs.db"
    monkeypatch.setenv("QT_OPS_DB", str(ops_db))
