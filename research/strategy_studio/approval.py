...
def autonomous_paper_approve(spec: S.StrategySpec, ev: EvidenceReport, readiness: dict, *,
                             current_state: str, max_allocation: float,
                             max_open_risk_pct: float, max_trades_per_day: int,
                             review_date: str, paper_requirements=(),
                             experiment_id: str = "EXP-006") -> tuple:
    """Approve a strategy for PAPER **autonomously** — the opt-in paper-autopilot path the
    user deliberately engaged. Returns (ApprovalRecord, new_state).

    It is exactly as strict about EVIDENCE as the human path (no red gate, no synthetic,
    survivor verdict only) — full paper autonomy lets the system trade REAL-data strategies
    on its own and blow up *simulated* money; it never lets it fabricate evidence. It is
    PAPER-ONLY and can never touch live: the lifecycle forbids the paper autopilot from the
    live-review transition, and the record's allowed_mode is always PAPER.
    """
    S.require_transition(current_state, S.APPROVED_FOR_PAPER, S.PAPER_AUTOPILOT)
    if not readiness or not readiness.get("can_run") or readiness.get("color") == "red":
        raise ApprovalRefused("evidence gate is red — cannot deploy to paper")
    if ev.is_synthetic:
        raise ApprovalRefused("evidence is synthetic (a software demo) — not market "
                              "evidence; the paper autopilot will not deploy it")
    if ev.verdict not in ("PROMOTE", "PASS"):
        raise ApprovalRefused(f"evidence verdict is '{ev.verdict}', not a survivor — "
                              "the paper autopilot will not deploy it")
    rec = ApprovalRecord(
        strategy_id=spec.strategy_id, version=spec.version, config_hash=spec.config_hash(),
        experiment_id=experiment_id, dataset_snapshot=spec.dataset_snapshot,
        evidence_verdict=ev.verdict, limitations=tuple(spec_limitations(readiness)),
        approval_timestamp=time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        approving_user=S.PAPER_AUTOPILOT, allowed_mode="PAPER",
        max_allocation=float(max_allocation), max_open_risk_pct=float(max_open_risk_pct),
        max_trades_per_day=int(max_trades_per_day), review_date=review_date,
        paper_evaluation_requirements=tuple(paper_requirements))
    return rec, S.APPROVED_FOR_PAPER
...
def activate_paper(record: ApprovalRecord, spec: S.StrategySpec, *, actor: str,
    ...
    if actor not in ("user", S.PAPER_AUTOPILOT):
        raise ApprovalRefused("only a user or the engaged paper autopilot may activate paper")
