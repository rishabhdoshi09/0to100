...
from research.edge005.constants import OUT_DIR
...
def write_all(stats: dict[str, Any], out_dir: Path | None = None) -> dict[str, str]:
    ...
    dec = (stats.get("deciles") or {}).get("P1") or {}
    inf = stats.get("inference") or {}
    crash = stats.get("crash") or {}
    u = stats.get("universe") or {}

    (out / "EDGE_005_DATA_INTEGRITY.md").write_text(
        "# EDGE-005 — Data Integrity\n\n"
        "Same store and PIT screens as EDGE-001+. P1 uses max(closes ≤ T) only.\n\n"
    ...
    (out / "EDGE_005_DECILES.md").write_text(
    ...
    (out / "EDGE_005_PORTFOLIO_RESULTS.md").write_text(
        "# EDGE-005 — Portfolio Results (P1 252d near-high, Top20 monthly)\n\n"
        + _md_table(["Metric", "P1 Top20 Monthly"], metrics) + "\n\n## By year\n\n"
    ...
    (out / "EDGE_005_TURNOVER_COSTS.md").write_text(
        "# EDGE-005 — Turnover and Costs\n\n"
    ...
    (out / "EDGE_005_CAPACITY.md").write_text(
    ...
    (out / "EDGE_005_WALK_FORWARD.md").write_text(
        "# EDGE-005 — Walk-Forward\n\n"
        "Blocks frozen before results. P1/Top20/monthly not retuned.\n\n"
    ...
    (out / "EDGE_005_REGIME_ANALYSIS.md").write_text(
        "# EDGE-005 — Regime (descriptive only)\n\nNo regime gate.\n\n"
        + _md_table(["Regime", "Months", "Mean net", "CAGR-like"], rrows) + "\n"
    )
    (out / "EDGE_005_RESULTS.md").write_text(
        "# EDGE-005 — Results\n\n"
    ...
    (out / "EDGE_005_DECISION.md").write_text(
        "# EDGE-005 — Decision\n\n"
        f"## `{d.get('label')}`\n\n"
        ...
        "Do not rescue with 63d high, laggards, or a scanner-style demote inside EDGE-005.\n"
    )
    return {p.stem: str(p) for p in out.glob("EDGE_005_*.md")}
