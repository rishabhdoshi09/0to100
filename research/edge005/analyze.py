"""EDGE-005 inference. Near-high slope + §17 robustness bar (predeclared)."""
...
from research.edge005.constants import DSR_N_TRIALS, LOG_DIR, OUT_DIR
...
def classify(stats: dict[str, Any]) -> dict[str, Any]:
    ...
    dec = stats.get("deciles", {}).get("P1") or stats.get("deciles", {}).get("M1") or {}
    ...
    if d10m1 is not None and d10m1 == d10m1 and d10m1 <= 0:
        failures.append("near_high_do_not_beat_far")
    ...
    p1, p3, lag = formulas.get("P1"), formulas.get("P3"), formulas.get("LAG")
    if p3 is not None and p1 is not None and p3 == p3 and p1 == p1 and p1 <= 0 and p3 > 0.02:
        failures.append("only_63d_works")
        notes.append("63d high is nearer short momentum. Do not switch inside EDGE-005.")
    if lag is not None and lag == lag and lag > 0 and (p1 is None or p1 <= 0):
        failures.append("only_laggards_work")
        notes.append("LAG is a laggard book; EDGE-004 already rejected short-horizon losers.")
    ...
    hard = {
        ...
        "deciles_not_ordered",
        "near_high_do_not_beat_far",
        "costs_destroy_edge",
    }
    if ("only_63d_works" in failures or "only_laggards_work" in failures) and not beat_ew:
        ...
        and "deciles_not_ordered" not in failures
        and "near_high_do_not_beat_far" not in failures
...
def analyse(artifacts: Path | None = None) -> dict[str, Any]:
    ...
    prim = periods.get("primary_P1_top20_monthly") or []
    ...
    sensitivities = {k: _spec_metrics(v, "net") for k, v in periods.items() if k != "primary_P1_top20_monthly"}
    formula_excess = {}
    keymap = {
        "P1": "primary_P1_top20_monthly",
        "P2": "sens_P2_top20_monthly",
        "P3": "sens_P3_top20_monthly",
        "LAG": "diag_LAG_top20_monthly",
    }
    ...
    deciles = {
        "M1": _decile_table(dec_raw.get("P1_decile") or []),
        "P1": _decile_table(dec_raw.get("P1_decile") or []),
        "P2": _decile_table(dec_raw.get("P2_decile") or []),
        "P3": _decile_table(dec_raw.get("P3_decile") or []),
    }
    ...
    (artifacts / "edge_005_stats.json").write_text(json.dumps(stats, indent=2, default=str))
    (OUT_DIR / "edge_005_stats.json").write_text(json.dumps(stats, indent=2, default=str))
    (OUT_DIR / "edge_005_manifest.json").write_text(json.dumps(manifest, indent=2, default=str))
    return stats
