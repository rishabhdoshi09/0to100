"""EDGE-006 inference. High-ADV slope + §17 robustness bar."""
...
from research.edge001.analyze import (
    _ann_from_monthly, _arr, _beta, _block_slice, _by_year, _capacity, _crash,
    _decile_table, _moments, _sharpe, _spec_metrics, _max_dd_from_rets,
)
from research.edge001.constants import block_of
from research.edge006.constants import DSR_N_TRIALS, LOG_DIR, OUT_DIR
...
def classify(stats: dict[str, Any]) -> dict[str, Any]:
    ...
    if d10m1 is not None and d10m1 == d10m1 and d10m1 <= 0:
        failures.append("liquid_do_not_beat_illiquid")
    if years:
        vals = list(years.values())
        if max(vals) / (sum(abs(v) for v in vals) or 1.0) > 0.80 and len(vals) >= 3:
    ...
    conf_reverse = (dev_ex is not None and conf_ex is not None and dev_ex == dev_ex and conf_ex == conf_ex
                    and dev_ex > 0.02 and conf_ex < -0.02)
    if conf_reverse:
        failures.append("confirmation_reverses_development")
    l0, l1 = formulas.get("L0"), formulas.get("L1")
    if l0 is not None and l1 is not None and l0 == l0 and l1 == l1 and l1 <= 0 and l0 > 0.02:
        failures.append("only_illiquids_work")
        notes.append("Illiquidity premium is a different hypothesis. Do not switch inside EDGE-006.")
    ...
    label = "RESEARCH-ONLY"
    hard = {"net_does_not_beat_ew_or_nifty", "deciles_not_ordered", "liquid_do_not_beat_illiquid", "costs_destroy_edge"}
    if "only_illiquids_work" in failures and not beat_ew:
        label = "MODIFY HYPOTHESIS"
    elif len(hard & set(failures)) >= 2:
        label = "REJECT"
    elif (beat_ew and not conf_reverse and "costs_destroy_edge" not in failures
          and "deciles_not_ordered" not in failures and "liquid_do_not_beat_illiquid" not in failures
          and "one_year_dominates" not in failures and valconf_n >= 12):
        ...
        label = "RESEARCH-ONLY"
    return {
        "label": label, "failures": failures, "notes": notes,
        "later_excess_ew": later_ex_ew, "later_excess_nifty": later_ex_nifty,
        "live_trading_authorised": False, "paper_trading_authorised": False,
...
def analyse(artifacts: Path | None = None) -> dict[str, Any]:
    ...
    prim = periods.get("primary_L1_top20_monthly") or []
    pm = _spec_metrics(prim, "net")
    pm.update({"by_year_net": _by_year(prim, "net"), "by_year_gross": _by_year(prim, "gross")})
    nifty_rets, ew_rets, net_rets = _arr([b.get("nifty") for b in bench]), _arr([b.get("ew_universe") for b in bench]), _arr([p.get("net") for p in prim])
    ...
    for name in ("development", "validation", "confirmation"):
        ...
        nr, er, nf = _arr([p.get("net") for p in sl]), _arr([b.get("ew_universe") for b in bsl]), _arr([b.get("nifty") for b in bsl])
        ...
        blocks[name] = m
    formula_excess = {}
    for key, col in (("L1", "primary_L1_top20_monthly"), ("L0", "diag_L0_top20_monthly"), ("L1_50", "sens_L1_top50_monthly")):
    ...
    for r in regime:
        bucket = {"STRONG_BULL": "bull", "BULL": "bull", "SIDEWAYS": "sideways",
                  "CORRECTION": "correction", "BEAR": "bear"}.get(str(r.get("regime") or ""), "unknown")
    ...
    stats = {
        "manifest": manifest, "primary": pm, "blocks": blocks,
        "sensitivities": {k: _spec_metrics(v, "net") for k, v in periods.items() if k != "primary_L1_top20_monthly"},
        "formula_excess_ew": formula_excess,
        "deciles": {"L1": _decile_table(dec_raw.get("L1_decile") or [])},
        "inference": inference, "crash": _crash(prim), "capacity": _capacity(prim),
        "regime": regime_out,
        "universe": {
    ...
    (artifacts / "edge_006_stats.json").write_text(json.dumps(stats, indent=2, default=str))
    (OUT_DIR / "edge_006_stats.json").write_text(json.dumps(stats, indent=2, default=str))
    (OUT_DIR / "edge_006_manifest.json").write_text(json.dumps(manifest, indent=2, default=str))
    return stats
