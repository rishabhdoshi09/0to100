...
class Supervisor:
    ...
    def _ensure_closed_market_decision_cycle(self) -> None:
        """Persist one current selector judgment set while cash entries are closed.

        Identity includes data, thesis and immutable calibration. The job cannot
        open a paper position and cannot create forward/counterfactual evidence.
        A changed thesis/calibration therefore earns a new audit judgment, while
        unchanged repeated supervisor ticks stay idempotent.
        """
        now_ist = self.deps.now_ist()
    ...
    def enqueue_due(self, now_ist=None):
        ...
        # Persist one canonical selector judgment for the current data/thesis/
        # calibration identity. This is operational truth only: no fill, no
        # forward evidence and no learning ingestion.
        ...
        try:
            ...
            # Resource Governor: learning/research above can consume already
            # produced evidence, but heavy historical replay must yield to due
            # or running official-data/current-market/forward-settlement work.
            # This prevents EOD snapshot refresh and replay from fighting for the
            # same disk/CPU on the operator's Mac while keeping the organisation
            # productive instead of globally pausing autonomy.
    ...
    def _incident(self, code, message, job=None):
        ...
        # Dialogue remains an append-only audit trail, but repeated identical
        # occurrences live in the dossier's occurrence_count instead of flooding
        # the stream. A material stage/progress/blocker change gets a new audit row.
