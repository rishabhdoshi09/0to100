...
from research.edge002.constants import OUT_DIR
...
def write_all(stats: dict[str, Any], out_dir: Path | None = None) -> dict[str, str]:
    ...
    dec = (stats.get("deciles") or {}).get("V1") or {}
    inf = stats.get("inference") or {}
    crash = stats.get("crash") or {}
    u = stats.get("universe") or {}

    (out / "EDGE_002_DATA_INTEGRITY.md").write_text(
        "# EDGE-002 — Data Integrity\n\n"
        "Same store and PIT screens as EDGE-001, including `live_on_session` "
        "(no stale last prints). Vol is computed on closes ≤ T only.\n\n"
        ...
        f"- Parent inconclusive study `{m.get('parent_inconclusive')}` (29-name, 20d L/S)\n"
    )
    ...
    (out / "EDGE_002_DECILES.md").write_text(
    ...
    (out / "EDGE_002_PORTFOLIO_RESULTS.md").write_text(
        "# EDGE-002 — Portfolio Results (V1 126d vol, Top20 monthly)\n\n"
        + _md_table(["Metric", "V1 Top20 Monthly"], metrics) + "\n\n## By year\n\n"
    ...
    (out / "EDGE_002_TURNOVER_COSTS.md").write_text(
        "# EDGE-002 — Turnover and Costs\n\n"
    ...
    (out / "EDGE_002_CAPACITY.md").write_text(
    ...
    (out / "EDGE_002_WALK_FORWARD.md").write_text(
        "# EDGE-002 — Walk-Forward\n\n"
        "Blocks frozen before results. V1/Top20/monthly not retuned.\n\n"
    ...
    (out / "EDGE_002_REGIME_ANALYSIS.md").write_text(
        "# EDGE-002 — Regime (descriptive only)\n\nNo regime gate.\n\n"
        + _md_table(["Regime", "Months", "Mean net", "CAGR-like"], rrows) + "\n"
    )
    (out / "EDGE_002_RESULTS.md").write_text(
        "# EDGE-002 — Results\n\n"
        ...
        f"Decile Spearman {_num(dec.get('spearman'))}.\n\n"
    ...
    (out / "EDGE_002_DECISION.md").write_text(
        "# EDGE-002 — Decision\n\n"
        f"## `{d.get('label')}`\n\n"
        ...
        "Do not rescue with stops, momentum overlays, or switching to V0/V3 after seeing results.\n"
    )
    return {p.stem: str(p) for p in out.glob("EDGE_002_*.md")}
