...
def _scan_state() -> dict[str, Any]:
    ...
    return {
        ...
        "latest_production_scan_ts_unix": ts,
        "latest_production_scan_ts_ist": _unix_to_ist_iso(ts),
        "n_results": len(raw.get("results") or []),
        "scanned_count": raw.get("count") or 0,
    }
...
def classify_empty_primary(scan: dict[str, Any], ledger: dict[str, Any]) -> dict[str, Any]:
    ...
    if not scan.get("scan_store_exists") or not latest:
        return {
            ...
            "severity": "info",
            "detail": (
                "No scan_store.json (or no timestamp). Protocol activated "
                f"{FORWARD_START_TS_IST}. Empty primary ledger is expected until "
                "a market-hours production scan runs after that instant."
            ),
        }
