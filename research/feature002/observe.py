...
def _frames_for(symbols: Sequence[str]) -> dict:
    frames = {}
    for sym in symbols:
        df = _hist(sym)
        if df is not None and len(df):
...
def _build_rs_table(symbols: Sequence[str], as_of: str):
    """Same rs_cs_v1 formula. FastRS if present, else sepa.rs.build_rs_table."""
    ...
    try:
        ...
        try:
            ...
            return build_rs_table(frames, as_of, DEFAULT_CONFIG, universe=list(frames))
        except Exception:
            return None


def try_observe_production_scan(
    cards: Sequence[Mapping[str, Any]] | None,
    *,
    last_scan_ts: float | None = None,
    path=None,
) -> dict[str, Any] | None:
    """Fail-open production hook. Never raises. Never mutates cards."""
    import os
    from pathlib import Path as _Path

    if path is None and os.environ.get("PYTEST_CURRENT_TEST"):
        path = _Path(os.environ.get("PYTEST_FEATURE002_DB") or "/tmp/feature002-pytest-shadow.db")
    out = None
    try:
        out = observe_production_scan(
            cards, source=PRIMARY_SOURCE, background=False, path=path,
        )
    except Exception as exc:
        _write_hook_event({"kind": "hook_exception", "error": str(exc)})
        out = None
    try:
        from research.feature002.watchdog import note_production_scan
        note_production_scan(n_cards=len(cards or []), last_scan_ts=last_scan_ts)
    except Exception:
        pass
    return out
