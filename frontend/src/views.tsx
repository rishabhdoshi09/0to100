  ...
  return (
    <section className="workspace-view">
      ...
      <div className="view-metrics">
        ...
        <MetricCard
          label="TELEGRAM"
          value={dashboard.data.telegram?.configured ? (dashboard.data.telegram.last_send_ok === false ? 'SEND FAILED' : (dashboard.data.telegram.listener_running ? 'OPTIONAL LIVE' : 'INBOX SILENT')) : 'NOT SET'}
          detail={dashboard.data.telegram?.last_error || dashboard.data.telegram?.note || 'The desk is this browser. Telegram is optional.'}
          tone={dashboard.data.telegram?.last_send_ok === false ? 'amber' : 'cyan'}
        />
      </div>
